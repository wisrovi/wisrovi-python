...
from wisrovi_lib.tutor_engine import TutorEngine
...
-   **{AUTHOR_INFO['name']}**  
    *{AUTHOR_INFO['title']} &bull; Badajoz, España*  
    ...
    [:octicons-mark-github-16: GitHub]({AUTHOR_INFO['github']}) &bull; [:octicons-globe-16: Sitio Web]({AUTHOR_INFO['website']}) &bull; [:octicons-package-16: PyPI]({AUTHOR_INFO['pypi']}) &bull; [:octicons-link-external-16: LinkedIn]({AUTHOR_INFO['linkedin']})
...
# ==============================================================================
def build_class_doc_page(meta: Dict[str, Any], class_idx: int) -> str:
    c_num = meta["course_num"]
    f_name = meta["folder_name"]
    c_cfg = next(c for c in COURSES_CONFIG if c["course_num"] == c_num)
    
    tutor_data = TutorEngine.get_class_content(c_num, class_idx) or {}
    ...
    theory = tutor_data.get("theory") or clean_html(meta.get("p4_theory", ""))
    metaphor = tutor_data.get("metaphor") or meta.get("metaphor", "")
    
    mermaid_code = tutor_data.get("mermaid") or ""
    if not mermaid_code.strip().startswith("```mermaid"):
        mermaid_code = f"```mermaid\n{mermaid_code}\n```"
        
    demo_code = tutor_data.get("demo_code") or clean_html(meta.get("p6_code", ""))
    playground_code = tutor_data.get("playground_code", "")
    
    pythonic_info = tutor_data.get("pythonic_tip", {})
    bad_code = pythonic_info.get("antipattern") or clean_html(meta.get("p7_bad_code", ""))
    good_code = pythonic_info.get("pythonic") or clean_html(meta.get("p7_good_code", ""))
    explanation = pythonic_info.get("explanation") or clean_html(meta.get("p7_tip", ""))
    
    challenge = tutor_data.get("challenge_prompt") or clean_html(meta.get("p9_challenge", ""))
    challenge_starter = tutor_data.get("challenge_starter", "")
    socratic_hints = tutor_data.get("socratic_hints", [])
    
    hints_md = ""
    for idx, hint in enumerate(socratic_hints, 1):
        hints_md += f"""??? info "💡 Pista Socrática {idx}"\n    {hint}\n\n"""
        
    quiz_items = tutor_data.get("quiz", [])
    quiz_md = ""
    if quiz_items:
        quiz_md = "## ❓ Auto-Evaluación Conceptual (Micro-Quiz)\n\n"
        for q_idx, q in enumerate(quiz_items, 1):
            quiz_md += f"**{q_idx}. {q['question']}**\n\n"
            for opt_idx, opt in enumerate(q['options']):
                is_correct = " *(Opción Correcta)*" if opt_idx == q['correct_index'] else ""
                quiz_md += f"- [ ] {opt}{is_correct}\n"
            quiz_md += f"\n> *Explicación Didáctica:* {q.get('explanation', '')}\n\n"
...
-   :material-lightbulb-on: **Metáfora Central:** *«{metaphor}»*
-   :material-laptop: **Wisrovi Studio (Local):** [🚀 Abrir Reto](http://127.0.0.1:8501/?course={c_num}&class={class_idx}) &bull; [👨‍🏫 Modo Tutor](http://127.0.0.1:8501/tutor?course={c_num}&class={class_idx})
...
[![Abrir en Studio Local](https://img.shields.io/badge/Wisrovi_Studio-Abrir_en_Local_(127.0.0.1%3A8501)-0284c7?logo=python&logoColor=white)](http://127.0.0.1:8501/?course={c_num}&class={class_idx})
...
!!! note "🌟 Modelo Mental de la Sesión: «{metaphor}»"
...
!!! tip "⚡ Resolución Híbrida en 1 Clic (Local + Web)"
    Si tienes ejecutando `wisrovi ui` en tu terminal local, puedes [🚀 Abrir este Reto directamente en tu Studio Local (127.0.0.1:8501)](http://127.0.0.1:8501/?course={c_num}&class={class_idx}) para escribir tu código con auto-formateo AST, inspeccionar variables en el Heap/Stack y evaluarlo con pruebas en tiempo real.
    ...
    for c_cfg in COURSES_CONFIG:
        ...
        for idx, meta in enumerate(c_classes, start=1):
            ...
            with open(class_page_path, "w", encoding="utf-8") as f:
                f.write(build_class_doc_page(meta, idx))
