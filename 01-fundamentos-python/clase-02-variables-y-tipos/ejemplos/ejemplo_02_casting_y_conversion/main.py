...
def calcular_precio_total(
    ...
    total_con_impuesto: float = subtotal * (
        1 + (impuesto_porcentaje / 100)
    )  # eg. subtotal + IVA (21%)
...
total_pagar = calcular_precio_total(
    entrada_precio,
    entrada_cantidad,
)
...
print(
    f"Total calculado con IVA:  ${total_pagar:.2f} (Tipo retornado: {type(total_pagar).__name__})"
)
